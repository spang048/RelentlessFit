...
// "Bfast" keeps the 4-column grid from overflowing on narrow phones
...
// ── Inline suggestion list (no absolute positioning → no viewport overflow) ──
      ...
      {results.map((item, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <span className="text-xs font-bold text-[#1B72CC] bg-blue-50 px-1.5 py-0.5 rounded">
              {item.calories} cal
            </span>
            {item.source === 'usda' && (
              <span className="text-[10px] text-slate-400 bg-slate-100 px-1 py-0.5 rounded">USDA</span>
            )}
          </div>
        </button>
      ))}
...
function ServingsRow({
  ...
  return (
    <div className="bg-blue-50 rounded-lg px-3 py-2 mt-1 space-y-1.5">
      <p className="text-xs text-slate-500">
        Servings
        {servingLabel && <span className="text-slate-400 ml-1">({servingLabel})</span>}
      </p>
      <div className="flex gap-1 flex-wrap">
        ...
        <input
          ...
          step="0.25"
          min="0.25"
          className="w-14 px-1.5 py-1 border border-slate-200 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-[#1B72CC] bg-white"
        />
      </div>
    </div>
  )
...
export default function AddFoodModal({ onClose, onSaved, defaultDate }: Props) {
  ...
  const [activeItemSugg, setActiveItemSugg] = useState<{ idx: number; results: FoodSuggestion[] } | null>(null)

  const supabase = createClient()

  // ── Food search: instant local + debounced USDA ──────────────────────────────
  useEffect(() => {
    // Instantly show local results
    setSuggestions(
      searchFoods(foodName).map(f => ({ ...f, source: 'local' as const }))
    )
  }, [foodName])
  ...
  function handleFoodNameChange(value: string) {
    setFoodName(value)
    if (baseCalories !== null) { setBaseCalories(null); setServingLabel('') }
  }
  ...
  async function handleFoodSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('rf_food_entries').insert({
      ...
      food_name: foodName.trim(), calories: parseInt(calories),
  ...
  async function logMeal(meal: SavedMeal) {
    ...
    const loggedAt = defaultDate
      ? new Date(defaultDate + 'T12:00:00').toISOString()
      : new Date().toISOString()
    await supabase.from('rf_food_entries').insert(
      meal.items.map(item => ({
        user_id: user.id, logged_at: loggedAt, meal_type: mealType,
    ...
    setLoggingMeal(null); onSaved(); onClose()
  }
  ...
  function updateItemFoodName(idx: number, value: string) {
    setNewMealItems(prev => prev.map((it, i) =>
      i === idx ? { ...it, food_name: value, baseCalories: null, servingLabel: '' } : it
    ))
    const results = value.trim()
      ? searchFoods(value).map(f => ({ ...f, source: 'local' as const }))
      : []
    setActiveItemSugg(results.length > 0 ? { idx, results } : null)
  }

  function updateItemCalories(idx: number, value: string) {
    setNewMealItems(prev => prev.map((it, i) => i === idx ? { ...it, calories: value } : it))
  }
  ...
  function updateItemServings(idx: number, value: string) {
    setNewMealItems(prev => prev.map((it, i) => {
      ...
      return {
        ...it,
        servings: value,
        calories: it.baseCalories !== null ? String(Math.round(it.baseCalories * s)) : it.calories,
      }
    }))
  }

  function selectItemSuggestion(idx: number, food: FoodSuggestion) {
    setNewMealItems(prev => prev.map((it, i) =>
      i === idx
        ? { food_name: food.name, calories: String(food.calories), baseCalories: food.calories, servings: '1', servingLabel: food.serving }
        : it
    ))
  ...
  async function saveNewMeal() {
    ...
    await supabase.from('rf_meals').insert({
      user_id: user.id, name: newMealName.trim(), items,
      total_calories: items.reduce((s, i) => s + i.calories, 0),
    })
    setSavingMeal(false); setShowCreate(false)
    setNewMealName(''); setNewMealItems([{ food_name: '', calories: '', baseCalories: null, servings: '1', servingLabel: '' }])
    setActiveItemSugg(null); loadMeals()
  }

  function cancelCreate() {
    setShowCreate(false); setNewMealName('')
  ...
  // ── Modal shell ──────────────────────────────────────────────────────────────
  // Use a fixed backdrop + a separate absolutely-positioned card so the card
  // is bound by inset-x-0 (left:0; right:0) — guaranteed to be exactly
  // viewport-width with no flex-centering quirks that can cause overflow on iOS.
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Card container:
          Mobile → absolute bottom-0 left-0 right-0 (exactly screen-width)
          Desktop → flex center with max-w-md                              */}
      <div className="absolute inset-x-0 bottom-0 md:inset-0 md:flex md:items-center md:justify-center md:p-4">
        <div
          className="relative bg-white w-full md:max-w-md md:rounded-2xl rounded-t-2xl shadow-xl flex flex-col overflow-hidden"
        ...
        >
          ...
          {/* ── Scrollable body ────────────────────────────────────────────────── */}
          <div className="overflow-y-auto overflow-x-hidden flex-1 px-4 pb-5 pt-3">
              ...
              <form onSubmit={handleFoodSubmit} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Search USDA database or type…"
                    required
                    autoFocus
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B72CC]"
                  />
                  {showSuggestions && (
                ...
                {baseCalories !== null && (
                  <ServingsRow servings={servings} servingLabel={servingLabel} onChange={setServings} />
                )}
                      ...
                      <div key={idx} className="space-y-1">
                        <div className="flex gap-2 items-center">
                          <input
                            ...
                            onFocus={() => {
                              const r = searchFoods(item.food_name).map(f => ({ ...f, source: 'local' as const }))
                              setActiveItemSugg(r.length > 0 ? { idx, results: r } : null)
                            }}
                            onBlur={() => setTimeout(() => setActiveItemSugg(null), 150)}
                            placeholder="Food name"
                            className="flex-1 px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B72CC]"
                          />
                          <input
                            type="number"
                            value={item.calories}
                            ...
                            placeholder="Cal"
                            min="0"
                            className="w-[60px] px-2 py-2 border border-slate-200 rounded-lg text-center focus:outline-none focus:ring-2 focus:ring-[#1B72CC]"
                          />
